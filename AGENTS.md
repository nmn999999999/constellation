...
### 5. 编码器对比（2026-08-12 实测，114 帧/1752 字节/4 数据帧+crossfade）

| 编码器 | 档位 | 文件大小 | 解码帧/114 | 数据恢复 |
|--------|------|----------|-----------|----------|
| H.264  | CRF 23 | 517KB | 93 | ✅ |
| H.265  | CRF 34 | 136KB | 93 | ✅ |
| H.265  | CRF 38 | 88KB  | 22 | ❌ |
| VP9    | CRF 60 | 61KB  | 84 | ✅ |
| AV1    | CRF 50 | 61KB  | 99 | ✅ |

- VP9/AV1 极压（~60KB/3.8s）仍可完整恢复，比 H.264/H.265 抗压得多
- crossfade 过渡帧本身不可解码（预期），且过渡后数帧出现短暂 CrcMismatch，
  靠每数据帧冗余兜底不影响最终恢复
